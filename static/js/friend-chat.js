...
async function requestFriend() {
    ...
    if (!nickname) { resultEl.innerHTML = `<span style="color:#dc2626">닉네임을 입력해주세요.</span>`; return; }
    try {
...
async function handleFriendAction(targetId, action) {
    ...
    try {
        ...
        if (res.ok) {
            showToast(action === 'accept' ? '친구가 되었어요! 🎉' : '요청을 거절했습니다.', action === 'accept' ? 'success' : '');
            const item = document.getElementById(`req-item-${targetId}`);
            if (item) {
                item.style.opacity = '0';
                item.style.transition = 'opacity 0.3s';
                setTimeout(() => { item.remove(); refreshPanelData(); }, 300);
            }
        }
...
function closeConfirmModal() {
    ...
    setTimeout(() => { bg.style.display = 'none'; pendingDeleteFriendId = null; }, 250);
}
...
async function _loadHistory(targetId, winEl) {
    ...
    try {
        ...
        msgBox.innerHTML = '';
        if (history.length === 0) {
            msgBox.innerHTML = '<div style="text-align:center;color:#ccc;font-size:0.82rem;padding:30px 0">아직 대화가 없어요.<br>먼저 인사를 건네보세요! 👋</div>';
        } else {
            history.forEach((m) => _appendMessage(targetId, m.message, m.sender_id == currentUserId ? 'me' : 'other'));
        }
    } catch (e) {
        msgBox.innerHTML = '<div style="text-align:center;color:#dc2626;font-size:0.82rem;padding:20px 0">대화 기록을 불러오지 못했습니다.</div>';
    }
...
function _startHeartbeat() {
    ...
    heartbeatTimer = setInterval(() => {
        if (globalSocket && globalSocket.readyState === WebSocket.OPEN) {
            ...
            try { globalSocket.send(JSON.stringify({ type: 'ping' })); } catch (e) {}
        }
    }, 25000); // 25초마다 ping
}
...
function initFriendChatPanel() {
    ...
    if (!currentUserId) {
        const listContainer = document.getElementById('panel-list-container');
        if (listContainer) listContainer.innerHTML = `<div class="empty-state"><div class="empty-icon">🔐</div>로그인 후 친구 기능을<br>이용할 수 있어요.</div>`;
        const reqContainer = document.getElementById('panel-req-container');
        if (reqContainer) reqContainer.innerHTML = `<div class="empty-state"><div class="empty-icon">🔐</div>로그인 후 이용해주세요.</div>`;
        return;
    }
